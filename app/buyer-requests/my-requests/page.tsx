...
type ResponseStatus = SellerResponse["status"];

const RESPONSE_STATUSES: Array<{
...
export default function MyBuyerRequestsPage() {
  ...
  const [responseActionId, setResponseActionId] =
    useState("");
  ...
  async function loadRequests() {
    ...
    const loadedRequests =
      (requestData || []) as BuyerRequest[];

    setRequests(loadedRequests);

    const requestIds = loadedRequests.map(
      (request) => request.id
    );
    ...
    try {
      ...
      setResponses(
        (result.responses || []) as SellerResponse[]
      );
    } catch (error) {
      console.error(
        "Buyer response load error:",
        error
      );
  ...
  async function updateResponseStatus(
    ...
    if (status === "accepted") {
      const confirmed = window.confirm(
        "Accept this supplier's response? The seller will be emailed and any previously accepted response will move back to Shortlisted."
      );
      ...
      if (status === "accepted") {
        alert(
          result.notificationSent
            ? "Response accepted. The seller has been notified by email."
            : "Response accepted. The status was saved, but the seller email could not be sent."
        );
      }
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-4 text-lg text-slate-600">
              Manage your requests and review private
        ...
        {requests.length === 0 ? (
          <div className="mt-10 rounded-3xl border border-slate-300 bg-white p-10 text-center shadow-sm">
            <h2 className="text-3xl font-extrabold">
              You haven’t posted a buyer request
              yet
            </h2>

            <p className="mt-4 text-slate-600">
              Tell NorthStock suppliers what
              commercial inventory your business
              needs.
            </p>
                ...
                >
                        ...
                        <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-green-200 bg-green-50 p-5 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <p className="font-extrabold text-green-900">
                              Accepted{" "}
                              {acceptedResponse.company_name ||
                                "supplier"}
                            </p>

                            <p className="mt-1 text-sm text-green-800">
                              Once the purchase is
                              arranged, close this request
                              so other suppliers know it is
                              complete.
                            </p>
                          </div>
                          ...
                          <button
                            ...
                            className="shrink-0 rounded-xl bg-green-700 px-5 py-3 font-extrabold text-white disabled:opacity-50"
                          >
                            Mark Fulfilled
                          </button>
                        </div>
                  ...
                  <div className="bg-slate-50 p-7 md:p-8">
                    ...
                    {requestResponses.length === 0 ? (
                      <p className="mt-4 text-slate-600">
                        No suppliers have responded to
                        this request yet.
                      </p>
                    ) : (
                          ...
                          (response) => (
                            ...
                            >
                              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                ...
                                <span
                                  className={`rounded-full px-3 py-2 text-xs font-extrabold capitalize ${getResponseStatusStyle(
                                    response.status
                                  )}`}
                                >
                                  {response.status}
                                      ...
                                      (option) => {
                                        ...
                                        return (
                                          <button
                                            ...
                                            className={`rounded-xl px-4 py-3 text-sm font-bold transition disabled:cursor-not-allowed ${
                                              selected
                                                ? `${getResponseStatusStyle(
                                                    option.value
